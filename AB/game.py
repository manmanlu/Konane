	...
	def evaluateLeafNode(self, node): #evalutes the leaf Node: number of avaliable max moves - number of avaliable not max move
		...
		if node.isMax:
			if Konane(self.size).isLost(node.currentBoard, node.mode):
				...
				evaNum  = len(self.getChildrenForANode(node)) - len(self.getChildrenForANode(notMeNode))
			...
			#	return float("inf")
			#else:
			evaNum= len(self.getChildrenForANode(notMeNode)) - len(self.getChildrenForANode(node))
			while(node.parent!=None):
				node = node.parent
			node.score= node.score+1
			return evaNum
